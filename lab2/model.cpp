...
void model::move()
{
    QPair<short,short> crd;
    crd.first=1;
    crd.second=1;
    gotoxy(crd.first,crd.second);
    char ch=0;
    while (ch!=32)
    {
        ch = _getch();
            ...
            case 'w':
        {
            if (crd.second > 1)
                crd.second-=2;
           /* if (board[crd.first][crd.second]==none)*/
            gotoxy(crd.first,crd.second);
            break;
        }
            case 's':
        {
            if (crd.second < board.size()*2 -1)
                crd.second+=2;
            gotoxy(crd.first,crd.second);
            break;
        }
    ...
    change_player();
    taken++;
    board[crd.first/2][crd.second/2]=_player;
}
